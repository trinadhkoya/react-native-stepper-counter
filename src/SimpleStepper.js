...
import {normalize} from "../../../src/_helpers/_functions";
...
export default class SimpleStepper extends Component {
  ...
  _renderAdd = (value, _renderAdd, textStyle) => {
    if (renderText) {
      return _renderAdd(value);
    }
    return <Text style={textStyle}>{value}</Text>;
  };
  ...
  render() {
    ...
    const isLeft = showText && value > 0 && textPosition == TEXT_POSITION.left;
    const isCenter = showText && value > 0 && textPosition == TEXT_POSITION.center;
    const isRight = showText && value > 0 && textPosition == TEXT_POSITION.right;

    const decStep = value > 0 ? <Step
    ...
    /> : <Step
        ...
        onPress={this.incrementAction}
        disabled={hasReachedMax || disabled}
        renderImage={() => this._renderText("ADD", renderText, [textStyle, {paddingRight: 5, fontSize: normalize(12)}])}
        imageStyle={incrementImageProps.style}
        imageOpacity={incrementImageProps.opacity}
        imageSource={incrementImageProps.source}
    />
